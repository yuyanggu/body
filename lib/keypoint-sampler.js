...
// ============================================================
// RawKeypointSampler — only the 17 MoveNet keypoints, no fill
// ============================================================

export class RawKeypointSampler {
    constructor() {
    ...
    _kpPos(kp) {
        if (kp.confidence >= CONFIDENCE_THRESHOLD) {
            return { x: kp.position3D.x * BODY_SCALE, y: kp.position3D.y * BODY_SCALE, z: kp.position3D.z * BODY_SCALE };
        }
        return null;
    }
    ...
    update(bodyState) {
        ...
        // 17 raw keypoints
        for (let i = 0; i < 17; i++) {
            points.push(this._kpPos(kp[i]) || OFF);
        }

        // Interpolated points along each skeleton bone (8 samples per bone)
        const BONE_SAMPLES = 8;
        for (const [a, b] of SKELETON_CONNECTIONS) {
            const pa = this._kpPos(kp[a]);
            const pb = this._kpPos(kp[b]);
            if (pa && pb) {
                for (let s = 1; s <= BONE_SAMPLES; s++) {
                    const t = s / (BONE_SAMPLES + 1);
                    points.push({
                        x: pa.x + (pb.x - pa.x) * t,
                        y: pa.y + (pb.y - pa.y) * t,
                        z: pa.z + (pb.z - pa.z) * t,
                ...
                for (let s = 0; s < BONE_SAMPLES; s++) points.push(OFF);
            }
        }

        // Fill remaining slots by cycling through valid skeleton points
        const validPoints = points.filter(p => p.y > -100);
        const fillSource = validPoints.length > 0 ? validPoints : [OFF];
        while (points.length < totalSlots) {
            points.push(fillSource[points.length % fillSource.length]);
        }
...
// ============================================================
// KeypointSampler — 256-slot interpolated body fill
// ============================================================
