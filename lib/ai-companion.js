...
export class AICompanion {
    ...
    async _generateReflection(trigger, state, metrics) {
        ...
        try {
            ...
            const useProxy = !this.voice.apiKey;
            const url = useProxy ? '/api/chat' : 'https://api.openai.com/v1/chat/completions';
            const headers = { 'Content-Type': 'application/json' };
            if (!useProxy) headers['Authorization'] = `Bearer ${this.voice.apiKey}`;
