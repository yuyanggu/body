...
export function processKeypoints(raw) {
    ...
    for (let i = 0; i < 17; i++) {
        ...
        const rawConf = r.score || 0;
        if (rawConf < CONFIDENCE_THRESHOLD) {
            // Hold last known position with decaying confidence instead of dropping
            kp.confidence = Math.max(kp.confidence * 0.95, 0);
            if (kp.confidence >= CONFIDENCE_THRESHOLD) trackAny = true;
            kp._recoveryBlend = 0; // reset blend when lost
            continue;
        }
        // When reappearing after being lost, blend in very gradually over many frames
        if (kp._recoveryBlend === undefined) kp._recoveryBlend = 1;
        if (kp.confidence < CONFIDENCE_THRESHOLD) {
            kp._recoveryBlend = 0;
        }
        kp._recoveryBlend = Math.min(kp._recoveryBlend + 0.02, 1);
        const smooth = 1 - (1 - SMOOTHING_FACTOR) * kp._recoveryBlend;
        kp.confidence = rawConf;
        trackAny = true;
        const nx = r.x / videoWidth, ny = r.y / videoHeight;
        kp.raw.x = nx; kp.raw.y = ny;
        const px = kp.smoothed.x, py = kp.smoothed.y;
        kp.smoothed.x = px * smooth + nx * (1 - smooth);
        kp.smoothed.y = py * smooth + ny * (1 - smooth);
